...
for length in hamming_stats.keys():
    ...
    width = 0.20
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - (3/2)*width, correct, width, label='Corregido')
    rects2 = ax.bar(x - width/2, wrong, width, label='No Corregido')
    rects3 = ax.bar(x + width/2, correct2, width, label='Errores detectados')
    rects4 = ax.bar(x + (3/2)*width, wrong2, width, label='Errores sin detectar')
    ax.set_ylabel('Cantidad')
    ax.set_xlabel('Cantidad de Errores por cada 100 Bits')
...
for length in fletcher_stats.keys():
    ...
    width = 0.20
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - (3/2)*width, correct, width, label='Error Detectado')
    ...
    rects4 = ax.bar(x + (3/2)*width, wrong2, width, label='Si hay error y no se detecto')
    ax.set_ylabel('Cantidad')
    ax.set_xlabel('Cantidad de Errores por cada 100 Bits')
